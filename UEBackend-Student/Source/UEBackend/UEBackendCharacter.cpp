...
AUEBackendCharacter::AUEBackendCharacter()
{
...
void AUEBackendCharacter::BeginPlay()
{
	// Call the base class  
	Super::BeginPlay();
	...
	if(WBP_ChatWidget)
	{
		ChatWidgetInstance = CreateWidget<UChatWidget>(GetWorld(), WBP_ChatWidget);
...
void AUEBackendCharacter::AddAbility(FString Name, FString IconPath)
{
	...
	{
		for (FGameplayAbility& GameplayAbility : PlayerData.Abilities)
		{
			GameplayAbility.IsEquipped = false;
		}
	}
...
void AUEBackendCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		//Moving
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered, this, &AUEBackendCharacter::Move);

		//Looking
		EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered, this, &AUEBackendCharacter::Look);

	}

}

void AUEBackendCharacter::Move(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AUEBackendCharacter::Look(const FInputActionValue& Value)
{
	// input is a Vector2D
